...
function allRandomClick() {
    shuffleArray(document.querySelectorAll("[class*=TopicsPage__Main]  button")).forEach(each => each.click());
}
...
var click = setInterval(allRandomClick);
var send = setInterval(sendKeyClick, Math.random() * (10000 - 1000) + 1000);
